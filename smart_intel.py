...
try:
    from openai import OpenAI  # optional; safe if missing
except Exception:
...
# ------------------ small OpenAI helpers (optional) ------------------
def _oai_chat_json(system: str, user: str) -> Dict[str, Any]:
...
def _oai_chat_text(system: str, user: str) -> str:
    if not (OpenAI and OPENAI_API_KEY): return ""
    try:
        cli = OpenAI(api_key=OPENAI_API_KEY)
        out = cli.chat.completions.create(
...
def _normalize(x: str) -> str: return re.sub(r"\s+", " ", (x or "").strip().lower())
...
def analyze_guest_intent(message: str, context: Dict[str, Any]) -> Dict[str, Any]:
    sys = ("Analyze a vacation-rental guest message. Return ONLY JSON with fields: "
    ...
    elif has("restaurant", "eat", "dinner", "coffee"):       pi = "food_recs"
...
def get_property_profile(listing_id: str) -> Dict[str, Any]:
    # Stub profile; replace with DB/API if desired
...
def _sem_rel(reply: str, guest_msg: str) -> bool:
    rt = set(re.findall(r"[a-z]{3,}", _normalize(reply)))
    gt = set(re.findall(r"[a-z]{3,}", _normalize(guest_msg)))
    if not rt or not gt: return True
    return (len(rt & gt) / max(1, len(gt))) >= 0.15

def _policy_issues(reply: str, ctx: Dict[str, Any]) -> List[str]:
    issues: List[str] = []
    if "early check in" in _normalize(reply) and (ctx.get("property_context") or {}).get("check_in_window") == "4pm-10pm":
...
PRIORITIES:
1) Urgent/safety first
2) Answer the question specifically
3) Add only relevant context
4) Provide next steps when useful

NEVER:
- Promise unavailable services
- Repeat the guest’s own words
- Use empty hospitality clichés
"""
...
def make_reply_smart(guest_message: str, meta_context: Dict[str, Any], history: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    # one short retry if invalid/low-confidence
    if not val["is_valid"] or val["confidence_score"] < 0.6:
        ...
        revised = _oai_chat_text(ENHANCED_SYSTEM_PROMPT, f"{guest_message}\n\nCRITIQUE:\n{critique}\nRevise briefly.")
...
def generate_autoreply(guest_message: str, context: Dict[str, Any], history: Optional[List[Dict[str, Any]]] = None) -> Tuple[str, Dict[str, Any]]:
    """
    Drop-in wrapper. Use this in your routes instead of your current composer if you want the new behavior.
    Returns (reply_text, meta) so existing call sites can keep using the first element.
    """
