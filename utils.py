# utils.py — full file with Google Distance Matrix support (updated)
...
def clean_ai_reply(text: str) -> str:
    ...
    - Normalize smart quotes/dashes/zero-width spaces.
    - Ensure spaces after punctuation WITHOUT breaking numbers like 4.4 or 3,757.
    - Remove boilerplate sign-offs and banned phrases.
    - Apply light contractions.
    - Avoid heavy mutations for list-style/bullet outputs.
    """
    ...
    trans = str.maketrans({
        "—": "—",   # keep em dash as is (we'll space around it below if needed)
    ...
    # Detect list/bullet style; be extra conservative for these
    is_listy = bool(re.search(r"(^|\n)\s*[-•]\s+", s)) or "Here are a few" in s

    # Space around em dashes (but don't overdo hyphens inside words)
    s = re.sub(r"\s*—\s*", " — ", s)

    if not is_listy:
        # Only add spaces after sentence punctuation when NOT between digits
        s = re.sub(r"(?<!\d)\.(?!\d)\s*", ". ", s)   # periods not in decimals
        s = re.sub(r"(?<!\d),(?!\d)\s*", ", ", s)    # commas not in thousands
        s = re.sub(r"\s*([!?;:])\s*", r"\1 ", s)     # generic punctuation spacing
        # Collapse extra spaces
        s = re.sub(r"\s{2,}", " ", s)
    else:
        # For bullet lists, just collapse excessive spaces and tidy line breaks
        s = re.sub(r"[ \t]+", " ", s)
    ...
    # Avoid trimming valid sentence ends for bullets; otherwise, a gentle tidy
    if not is_listy:
        s = s.rstrip(". ").strip()
